...
def show_estimate(phrases: list):
    ...
    for arch, data in sorted(
        stats["archetype_stats"].items(), key=lambda x: x[1]["count"], reverse=True
    ):
        settings = ARCHETYPE_VOICE_SETTINGS.get(
            arch, ARCHETYPE_VOICE_SETTINGS["default"]
        )
    ...
    console.print(
        f"  Duracao: [green]{stats['estimated_hours']:.1f} horas[/green] ({stats['estimated_minutes']:.0f} minutos)"
    )
...
def generate_dataset(
    phrases: list, output_dir: Path, dry_run: bool = False
    ...
    rate_config = RateLimitConfig(
        min_delay_seconds=2.5, max_delay_seconds=15.0, max_backoff_seconds=300.0
    )

    generator = ElevenLabsDatasetGenerator(
        voice_id=voice_id, rate_limit_config=rate_config
    )
    ...
    if progress_file.exists():
        try:
            with open(progress_file, "r") as f:
                ...
                console.print(
                    f"[yellow]Retomando de {start_index + 1}/{len(phrases)}...[/yellow]"
                )
        except Exception:
            pass
    ...
    with Progress(
        ...
        task = progress.add_task(
            "Gerando audios...", total=len(phrases), completed=start_index
        )
        ...
        for i, phrase_data in enumerate(phrases[start_index:], start_index + 1):
            ...
            if dry_run:
                progress.update(
                    task, completed=i, description=f"[DRY] {i}/{len(phrases)}"
                )
                continue
            ...
            if success:
                ...
                errors.append({"index": i, "phrase": text[:100], "error": error})
    ...
    metadata = DatasetMetadata(
        ...
        audio_format={"sample_rate": 22050, "channels": 1, "format": "wav"},
...
def train_coqui(dataset_dir: Path, output_name: str) -> bool:
    ...
    try:
        ...
        console.print("[green]Coqui configurado com sucesso![/green]")
...
def train_chatterbox(dataset_dir: Path, output_name: str) -> bool:
    ...
    try:
        ...
        console.print("[green]Chatterbox configurado com sucesso![/green]")
        console.print(f"  Diretorio: {reference_dir}")
        console.print(f"  Audio de teste: {test_output}")
