...
class RateLimiter:
    ...
    def on_success(self):
        ...
        self._current_delay = max(
            self.config.min_delay_seconds, self._current_delay * 0.9
        )
    ...
    def on_error(self) -> float:
        self._consecutive_errors += 1
        self._current_delay = min(
            self._current_delay * 1.5, self.config.max_backoff_seconds
        )
        return self._current_delay
...
class ElevenLabsDatasetGenerator:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "API key is required. Set ELEVENLABS_API_KEY or pass api_key parameter."
            )
    ...
    def estimate_cost(
        self, phrases: List[str], price_per_1k_chars: float = 0.03
    ) -> CostEstimate:
    ...
    def generate_sample(
        ...
        if not self.voice_id:
            raise ValueError(
                "Voice ID is required. Set ELEVENLABS_VOICE_ID or pass voice_id parameter."
            )
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if "429" in error_str or "rate" in error_str or "limit" in error_str:
                    ...
                    logger.warning(
                        f"429 Rate limit on attempt {attempt + 1}/{max_retries}. Waiting {wait_time:.1f}s..."
                    )
                    time.sleep(wait_time)
                    continue
                    ...
                    logger.warning(
                        f"Server error on attempt {attempt + 1}/{max_retries}. Waiting {wait_time:.1f}s..."
                    )
                    time.sleep(wait_time)
                    continue
    ...
    def generate_dataset(
        ...
        logger.info(
            f"Dataset generated: {successful}/{len(phrases)} samples, {total_duration:.1f}s total"
        )
